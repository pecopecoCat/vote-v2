...
import { QUESTION_MAX } from "./form/createVoteFormConstants";
...
type CreateQuestionStepProps = {
  initialQuestion?: string;
  onDecide: (question: string) => void;
  /** modal 内など、外側でヘッダーを出すとき */
  embedded?: boolean;
};

export default function CreateQuestionStep({
  initialQuestion = "",
  onDecide,
  embedded = false,
}: CreateQuestionStepProps) {
  const [question, setQuestion] = useState(initialQuestion);
  const [allCards, setAllCards] = useState<
    { question: string; countA: number; countB: number; commentCount: number }[]
  >([]);
  ...
  return (
    <main
      className={`mx-auto w-full flex-1 overflow-y-auto ${
        embedded ? "max-w-none px-4 pb-4 pt-4" : "max-w-lg px-[5.333vw] pb-[50px] pt-[10px]"
      }`}
    >
      <div className="rounded-2xl bg-white p-4">
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => isValid && onDecide(question.trim())}
            disabled={!isValid}
